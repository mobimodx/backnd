...
class ApiController extends Controller
{
    ...
    public function ImgToImg(Request $request)
    {
        ...
        if ($request->hasFile('image')) {
            ...
            $image = 'images' . time() . '.' . $file->getClientOriginalExtension();
        ...
        /*$output_file = file_get_contents($output);
        $output_filename = basename($output);
        $output_image = 'image' . time() . '-' . $output_filename;
        file_put_contents("/uploads/output_files/".$output_image, $output_file);
        $output_url = env('Image_url').'/uploads/output_files/'.$output_image;*/
        if($output){
            return json_encode(['status' => true,'output'=> $output]);
        }else{
            return json_encode(['status' => false,'output'=> $output,'message' => 'this job has been failed please try again.']);
